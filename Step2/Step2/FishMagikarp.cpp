 ...
 */
CFishMagikarp::CFishMagikarp(CAquarium *aquarium) : CItem(aquarium)
{
	mFishImage = unique_ptr<Bitmap>(Bitmap::FromFile(FishMagikarpImageName.c_str()));
	if (mFishImage->GetLastStatus() != Ok)
	{
		wstring msg(L"Failed to open ");
		msg += FishMagikarpImageName;
...
void CFishMagikarp::Draw(Gdiplus::Graphics *graphics)
{
	...
	graphics->DrawImage(mFishImage.get(),
		...
		mFishImage->GetWidth(), mFishImage->GetHeight());
}
