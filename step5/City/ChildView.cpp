...
BEGIN_MESSAGE_MAP(CChildView, CWnd)
	...
	ON_UPDATE_COMMAND_UI(ID_BIGTEN_RUNAWAY, &CChildView::OnUpdateBigtenRunaway)
	ON_COMMAND(ID_BIGTEN_RUNAWAY, &CChildView::OnBigtenRunaway)
...
/** Menu handler that adds a Michigan Fan */
void CChildView::OnBigtenMichiganfan()
{
	auto tile = make_shared<CMichiganFan>(&mCity);
	tile->SetLocation(InitialX, InitialY);
	mCity.Add(tile);
	Invalidate();
}

/** Menu handler that handles Big Ten>Play option */
void CChildView::OnBigtenPlay()
{
	mIsPlay = !mIsPlay;
}

/** Update the checkmark for the Big Ten>Play menu option */
void CChildView::OnUpdateBigtenPlay(CCmdUI *pCmdUI)
{
	CFanCounter visitor;
	mCity.Accept1(&visitor);
	int cnt = visitor.GetNumFans();
	if (cnt > 0)
	{
		pCmdUI->SetCheck(mIsPlay);
	}
	else
	{
		pCmdUI->Enable(mIsPlay); 
	}
}

/** Update the status (enable/disable) for the Big Ten>Run Away menu option */
void CChildView::OnUpdateBigtenRunaway(CCmdUI *pCmdUI)
{
	CFanCounter visitor;
	mCity.Accept1(&visitor);
	int cnt = visitor.GetNumFans();
...
/** Menu handler that handles Big Ten>Run Away option */
void CChildView::OnBigtenRunaway()
{
	mCity.SetRunAway(true);
	CPlayCounter visitor;
	mCity.Accept(&visitor);
	mCity.SetRunAway(false);
}

/** Update the status (enable/disable) for the Big Ten>New Season menu option */
...
/** Menu handler that handles Big Ten>New Season option */
void CChildView::OnBigtenNewseason()
{
	mCity.SetNewSeason(true);
	CPlayCounter visitor;
	mCity.Accept(&visitor);
	mCity.SetNewSeason(false);
}

/** Update the checkmark for the View>Mouse Tracks option */
void CChildView::OnUpdateViewMousetracks(CCmdUI *pCmdUI)
{
	pCmdUI->SetCheck(mShowHistory);	// Will check the menu option
}

/** Menu handler that handles View>Mouse Tracks option */
